...
describe("Editor.vue", () => {
  ...
  describe("publishing", () => {
    describe("clicking publish", () => {
      ...
      it("shows publish confirmation dialog upon clicking publish", async () => {
      ...
      it("confirming publish dialog starts publishing", async () => {
      ...
      it("share + play + embed buttons appear on publishing", async () => {
        ...
        expect(
          wrapper.find('[data-test="sharePlioButton"]').exists()
        ).toBeFalsy();
        expect(
          wrapper.find('[data-test="playPlioButton"]').exists()
        ).toBeFalsy();
        expect(
          wrapper.find('[data-test="embedPlioButton"]').exists()
        ).toBeFalsy();
    ...
    it("blurs the main screen and show dialog when published plio dialog is shown", async () => {
      ...
      expect(
        wrapper.find('[data-test="publishedDialog"]').exists()
      ).toBeFalsy();
  ...
  describe("element visibility", () => {
    it("shows only the video preview + video input field when video ID is not set", async () => {
      ...
      expect(
        wrapper.find('[data-test="videoLinkInput"]').exists()
      ).toBeTruthy();
    ...
    describe("video id added", () => {
      ...
      it("shows publish + home + preview buttons", async () => {
        // things that should not be visible
        expect(
          wrapper.find('[data-test="sharePlioButton]').exists()
        ).toBeFalsy();
        ...
        expect(
          wrapper.find('[data-test="publishButton"]').exists()
        ).toBeTruthy();
      });

      describe("org workspace", () => {
        beforeEach(async () => {
          // set active workspace to 'test'
          await store.dispatch("auth/setActiveWorkspace", "test");
        });

        it("also shows copy draft link button", async () => {
          // things that should not be visible
          expect(
            wrapper.find('[data-test="sharePlioButton]').exists()
          ).toBeFalsy();
          ...
          expect(
            wrapper.find('[data-test="analyseButton]').exists()
          ).toBeFalsy();
          ...
          expect(
            wrapper.find('[data-test="homeButton"]').exists()
          ).toBeTruthy();
          expect(
            wrapper.find('[data-test="publishButton"]').exists()
          ).toBeTruthy();
          expect(
            wrapper.find('[data-test="copyDraftButton"]').exists()
          ).toBeTruthy();
        });

        it("clicking on copy draft link button copies draft link", async () => {
          ...
          expect(
            wrapper.vm.getPlioDraftLink(wrapper.vm.plioId, wrapper.vm.org)
          ).toBe(draftLink);
        });
      });
    });
  ...
  describe("button functionality", () => {
    const mountWrapper = async (global = {}, data = {}, props = {}) => {
      wrapper.unmount();
      wrapper = mount(Editor, {
        global: global,
        data() {
    ...
    describe("preview", () => {
      ...
      it("clicking on the close button of preview closes the preview", async () => {
        ...
        await wrapper
          .find('[data-test="closePlioPreviewButton"]')
          .trigger("click");
